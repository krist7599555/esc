...
describe('lib sso', () => {
  ...
  it('should be defined', () => expect(sso.login).to.be.ok);

  it('normal flow', async () => {
    return expect(sso.login(PASS_USERNAME, PASS_PASSWORD)).notify(console.log)
  ...
  it('invalid username', async () => {
    for (const usr of [null, undefined, '', '*', '9999999', -1, NaN, '999999999999999']) {
      await expect(sso.login(usr, PASS_PASSWORD))
  ...
  it('invalid password', async () => {
    for (const pwd of [null, undefined, '', -1, NaN]) {
      await expect(sso.login(PASS_USERNAME, pwd))
        .eventually.rejectedWith(ValidationException, 'validation error')
